...
SD_Error SD_Init(void)
{
  ...
  LCD_DisplayStringLine(LCD_LINE_0, (uint8_t *)"Start initail SD Hardware");
  
  SD_LowLevel_Init();
  //_print("SD Hardware initial finish!");
	SDIO_DeInit();

	if (GPIO_ReadInputDataBit(SD_DETECT_GPIO_PORT, SD_DETECT_PIN) == SET)
	{
		LCD_DisplayStringLine(LCD_LINE_1, (uint8_t *)" NO CARD");
		return SD_ERROR;
	}
	...
	LCD_DisplayStringLine(LCD_LINE_1, (uint8_t *)" CARD INITIALIZE OK");
